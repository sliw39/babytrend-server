...
export function validateQuery(query: SurveyQuery): boolean {
    ...
    if (query.type && !["Change", "Someil", "Bain", "Repas", "Visite", "Medicament"].includes(query.type)) return false;
...
export function validateSurvey(survey: Survey): boolean {
    ...
    if (!["Change", "Someil", "Bain", "Repas", "Visite", "Medicament"].includes(survey.type)) return false;
